...
Real listings come from Jooble and/or JSearch (RapidAPI) when their API
keys are configured (JOOBLE_API_KEY / RAPIDAPI_KEY -- see .env.example);
...
error, bad key, quota), this falls back to the synthetic mock listings
below so JobSwiper never breaks -- see _generate_synthetic_jobs().
"""
...
# swipe deck spans the same range of entry-level work the synthetic
# jobs below do, rather than skewing toward whatever one keyword
# happens to return the most results.
...
def _fetch_live_jobs():
    ...
    call failed, so the caller can tell "no keys/all failed" apart from
    "a real search legitimately returned nothing" and fall back to mock
    data only in the former case."""
...
def _get_live_jobs_cached():
    ...
    if fresh is not None:
        ...
        return fresh
    # A failed refresh keeps serving the last good cache (even if
    # stale) rather than falling back to mock data underneath a user
    # who was already looking at real listings.
    return _live_jobs_cache["jobs"]
...
ALL_MOCK_JOBS = MOCK_JOBS + _generate_synthetic_jobs()


def _all_jobs():
    """Real listings first (if any source is configured and working),
    then the synthetic mock jobs as filler behind them -- so a thin
    result from the real APIs (or the small free-tier request budget
    running low) never leaves the swiper running dry, and there's
    always something real to swipe on top when a source is live."""
    live = _get_live_jobs_cached()
    if live:
        return live + ALL_MOCK_JOBS
